...
class KernelBridge {
  ...
  /**
   * Queries Windows HTTP Services (WinHTTP) proxy configurations for both current user IE settings and system default WinHTTP proxy.
   * Leverages winhttp.h / winhttp.dll (WinHttpGetIEProxyConfigForCurrentUser, WinHttpGetDefaultProxyConfiguration).
   * @returns {Promise<Object>}
   */
      ...
      execFile(
        ...
        (err, stdout, stderr) => {
          ...
          resolve({
            success: false,
            apiAvailable: false,
            error: stderr || err?.message || "Failed to query WinHTTP proxy configuration"
          });
        }
      );
    });
  ...
  /**
   * Opens a native WinHTTP session and queries protocol capabilities, decompression flags, HTTP/2/3 status, and connection timeouts.
   * Leverages winhttp.h / winhttp.dll (WinHttpOpen, WinHttpQueryOption).
   * @param {Object} [options]
   * @param {string} [options.userAgent]
   * @returns {Promise<Object>}
   */
  async getWinHttpSessionStatus(options = {}) {
    const agent = String(options.userAgent || options.agent || "GeminiSuperSystem/1.0");

    return new Promise((resolve) => {
      execFile(
        this.binPath,
        ["winhttp_session_status", agent],
        { timeout: 15000, encoding: "utf8" },
        (err, stdout, stderr) => {
          if (stdout && stdout.trim()) {
            try {
          ...
          resolve({
            success: false,
            apiAvailable: false,
            error: stderr || err?.message || "Failed to inspect WinHTTP session status"
          });
        }
      );
    });
  ...
  /**
   * Decomposes and parses a raw URL into canonical WinHTTP components (scheme, host, port, credentials, path, query/hash).
   * Leverages winhttp.h / winhttp.dll (WinHttpCrackUrl).
   * @param {Object} [options]
   * @param {string} [options.url]
   * @returns {Promise<Object>}
   */
  async crackWinHttpUrl(options = {}) {
    const rawUrl = String(options.url || options.rawUrl || "");

    return new Promise((resolve) => {
      execFile(
        this.binPath,
        ["winhttp_url_crack", rawUrl],
        { timeout: 15000, encoding: "utf8" },
        (err, stdout, stderr) => {
          if (stdout && stdout.trim()) {
            try {
          ...
          resolve({
            success: false,
            apiAvailable: false,
            error: stderr || err?.message || "Failed to crack WinHTTP URL"
          });
        }
      );
    });
  ...
  /**
   * Resolves Web Proxy Auto-Discovery (WPAD via DHCP / DNS_A) proxy configuration for a specified target URL.
   * Leverages winhttp.h / winhttp.dll (WinHttpGetProxyForUrl).
   * @param {Object} [options]
   * @param {string} [options.targetUrl]
   * @returns {Promise<Object>}
   */
  async resolveWinHttpAutoProxy(options = {}) {
    const targetUrl = String(options.targetUrl || options.url || "https://www.microsoft.com");

    return new Promise((resolve) => {
      execFile(
        this.binPath,
        ["winhttp_autoproxy_resolve", targetUrl],
        { timeout: 20000, encoding: "utf8" },
        (err, stdout, stderr) => {
          if (stdout && stdout.trim()) {
            try {
          ...
          resolve({
            success: false,
            apiAvailable: false,
            error: stderr || err?.message || "Failed to resolve WinHTTP auto proxy"
          });
        }
      );
    });
