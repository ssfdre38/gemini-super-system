...
class GeminiSuperDashboard {
  ...
  start() {
    ...
    this.server = http.createServer(async (req, res) => {
        ...
        req.on("end", async () => {
          try {
            ...
            if (data.text) {
              ...
              throw new Error("Specify 'text', 'keys', 'hotkey', 'click', 'doubleClick', 'rightClick', 'drag', 'scroll', or 'focus'");
            }
      ...
      // 4e2. API Desktop Child Windows & Controls
...
<body>
  ...
  <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(360px, 1fr)); gap: 16px; margin-bottom: 20px;">
    ...
    <div class="control-deck" style="border-color: rgba(0, 229, 255, 0.35); margin-bottom: 0;">
      ...
      <div class="deck-row">
        <select id="desktop-window-select" class="deck-select" style="flex: 1; min-width: 220px;">
      ...
      <div class="deck-row" style="margin-top: 8px;">
        <input type="text" id="desktop-keys-input" class="deck-input" placeholder="Keystrokes to send (e.g. text, {ENTER}, ^s)..." />
        <button onclick="sendDesktopKeys()" class="secondary">⌨️ Send Keys</button>
      </div>
      <div id="desktop-preview-container" style="display: none; margin-top: 14px; background: #030407; border: 1px solid var(--panel-border); border-radius: 6px; padding: 12px;">
        ...
        <img id="desktop-preview-img" style="max-width: 100%; max-height: 360px; border-radius: 4px; border: 1px solid #222; display: block; margin: 0 auto;" src="" alt="Desktop Snapshot" />
      </div>
    </div>
