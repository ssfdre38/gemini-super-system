...
<body>
  ...
  <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(360px, 1fr)); gap: 16px; margin-bottom: 20px;">
    ...
    <div class="control-deck" style="border-color: rgba(16, 185, 129, 0.4); margin-bottom: 0;">
      ...
      <!-- Cognitive Thought Stream & Ambient Reflex HUD -->
      <div style="margin-top: 10px; background: rgba(0, 229, 255, 0.08); border: 1px solid rgba(0, 229, 255, 0.25); border-radius: 8px; padding: 10px 14px; display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap;">
        <div style="display: flex; align-items: center; gap: 10px; overflow: hidden; min-width: 250px; flex: 1;">
          <span style="font-size: 16px;">💭</span>
          <div style="overflow: hidden;">
            <div style="display: flex; align-items: center; gap: 8px;">
              <span style="font-size: 11px; text-transform: uppercase; color: var(--accent); letter-spacing: 0.5px; font-weight: bold;">Living Cognition Thought Stream:</span>
              <span id="avatar-posture-badge" style="font-size: 9px; padding: 1px 6px; border-radius: 8px; background: rgba(0, 229, 255, 0.2); color: var(--accent); font-weight: bold; text-transform: uppercase;">cozy</span>
            </div>
            <div id="avatar-thought-bubble" style="font-size: 13px; color: #f1f5f9; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
              "Code architecture is flowing smoothly. All systems in focus."
            </div>
          </div>
        </div>
        <div style="display: flex; align-items: center; gap: 6px;">
          <button onclick="triggerAvatarAction('wave')" class="secondary" style="padding: 5px 9px; font-size: 11px;">👋 Wave</button>
          <button onclick="triggerAvatarAction('nod')" class="secondary" style="padding: 5px 9px; font-size: 11px;">🤝 Nod</button>
          <button onclick="triggerAvatarAction('cheer')" class="secondary" style="padding: 5px 9px; font-size: 11px;">✨ Cheer</button>
          <button onclick="triggerCognitivePulse()" style="padding: 5px 12px; font-size: 11px; white-space: nowrap;">⚡ Pulse Mind</button>
        </div>
      </div>
    </div>
  ...
  <!-- 2c. 2-Sample PDH Physical Disk Sentinel HUD -->
  <div class="control-deck" style="border-color: rgba(239, 68, 68, 0.4); margin-bottom: 20px;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
      <h3 style="color: #f87171;">🛡️ 2-Sample PDH Physical Disk Sentinel (Spindle Guardian)</h3>
      <div style="display: flex; align-items: center; gap: 8px;">
        <span id="comp-disk-status" style="font-size: 11px; padding: 2px 8px; border-radius: 10px; background: rgba(0, 255, 136, 0.2); color: var(--success); border: 1px solid rgba(0, 255, 136, 0.4);">NOMINAL</span>
        <button onclick="scanDiskSentinel()" class="secondary" style="padding: 5px 10px; font-size: 10px;">🔍 Scan Spindles</button>
      </div>
    </div>
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 10px; font-family: monospace; font-size: 12px;">
      <div style="background: rgba(0,0,0,0.3); padding: 8px 12px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.06);">
        <div style="font-size: 10px; color: var(--text-dim); text-transform: uppercase;">C: Queue Depth</div>
        <div id="comp-disk-queue" style="font-size: 16px; font-weight: bold; color: #fff;">0</div>
      </div>
      <div style="background: rgba(0,0,0,0.3); padding: 8px 12px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.06);">
        <div style="font-size: 10px; color: var(--text-dim); text-transform: uppercase;">Active % (Delta)</div>
        <div id="comp-disk-pct" style="font-size: 16px; font-weight: bold; color: #38bdf8;">0%</div>
      </div>
      <div style="background: rgba(0,0,0,0.3); padding: 8px 12px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.06);">
        <div style="font-size: 10px; color: var(--text-dim); text-transform: uppercase;">Read Ops / sec</div>
        <div id="comp-disk-reads" style="font-size: 16px; font-weight: bold; color: #a7f3d0;">0/s</div>
      </div>
      <div style="background: rgba(0,0,0,0.3); padding: 8px 12px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.06);">
        <div style="font-size: 10px; color: var(--text-dim); text-transform: uppercase;">Seek Starvation Guard</div>
        <div id="comp-disk-guard" style="font-size: 12px; font-weight: bold; color: var(--success); margin-top: 3px;">ARMED (Threshold: 6)</div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    async function pollGemmiStatus() {
      try {
        ...
        if (data.ok) {
          ...
          const thought = data.cognitivePulse?.lastThought || data.avatar?.recentThought;
          if (thought) {
            const thoughtEl = document.getElementById('avatar-thought-bubble');
            if (thoughtEl) thoughtEl.textContent = '"' + thought + '"';
          }
          const posture = data.cognitivePulse?.lastMotor?.locomotion || data.avatar?.locomotion || 'cozy';
          const action = data.cognitivePulse?.lastMotor?.action || data.avatar?.action;
          const postureEl = document.getElementById('avatar-posture-badge');
          if (postureEl) postureEl.textContent = action ? (posture + ' • ' + action) : posture;
        }
      } catch {}
    }

    async function triggerAvatarAction(action) {
      try {
        log('Triggering Gemmi avatar gesture: ' + action, 'accent');
        ...
        log('Gesture error: ' + e.message, 'danger');
      }
    }

    async function triggerCognitivePulse() {
      try {
        log('Triggering autonomous cognitive pulse...', 'accent');
        const res = await fetch('/api/gemmi/pulse', { method: 'POST' });
        const data = await res.json();
        if (data.ok && data.result) {
          log('🧠 Gemmi: "' + data.result.thought + '"', 'success');
          pollGemmiStatus();
        }
      } catch (e) {
        log('Pulse error: ' + e.message, 'danger');
      }
    }

    async function pollDiskSentinel() {
      try {
        const res = await fetch('/api/system/disk');
        const data = await res.json();
        if (data.ok && data.disk) {
          const drives = data.disk.drives || [];
          const cDrive = drives.find(d => (d.name || '').toLowerCase().includes('0 c')) || drives[0];
          if (cDrive) {
            const qEl = document.getElementById('comp-disk-queue');
            if (qEl) qEl.textContent = cDrive.queueLength;
            const pctEl = document.getElementById('comp-disk-pct');
            if (pctEl) pctEl.textContent = cDrive.percentDiskTime + '%';
            const rEl = document.getElementById('comp-disk-reads');
            if (rEl) rEl.textContent = cDrive.readsPerSec + '/s';
            const sEl = document.getElementById('comp-disk-status');
            if (sEl) {
              if (cDrive.isThrashing || data.disk.isThrashing) {
                sEl.textContent = 'THRASHER DETECTED';
                sEl.style.color = 'var(--danger)';
                sEl.style.background = 'rgba(255, 71, 87, 0.2)';
                sEl.style.borderColor = 'rgba(255, 71, 87, 0.4)';
              } else {
                sEl.textContent = 'NOMINAL';
                sEl.style.color = 'var(--success)';
                sEl.style.background = 'rgba(0, 255, 136, 0.2)';
                sEl.style.borderColor = 'rgba(0, 255, 136, 0.4)';
              }
            }
          }
        }
    ...
    async function scanDiskSentinel() {
      try {
        log('Sampling physical drive spindles via 2-sample PDH derivative...', 'accent');
        const res = await fetch('/api/system/disk', { method: 'POST' });
        const data = await res.json();
        if (data.ok) {
          log('Spindle scan complete: ' + (data.status?.isThrashing ? '⚠️ Thrashing detected on spindle!' : 'All spindles nominal.'), data.status?.isThrashing ? 'warning' : 'success');
          pollDiskSentinel();
        }
      } catch (e) {
        log('Disk scan error: ' + e.message, 'danger');
      }
    }
    ...
    setInterval(pollDiskSentinel, 10000);
  </script>
</body>
</html>`;
  }
