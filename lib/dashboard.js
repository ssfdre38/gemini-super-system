        ...
        req.on("end", async () => {
          try {
            ...
            if (data.keys) {
              ...
              result = await getDesktopBridge().clickWindow(data.title, data.click.x, data.click.y);
            } else {
              throw new Error("Specify 'keys' or 'click' in payload");
            }
            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(JSON.stringify(result));
          } catch (err) {
...
<body>
  ...
  <!-- Native Desktop Windows & Perception Deck -->
  <div class="control-deck" style="border-color: rgba(0, 229, 255, 0.35);">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
      <h3 style="color: var(--accent);">🖥️ Native Desktop Windows & Perception Deck (Sub-20ms On-Demand Snapshots)</h3>
      <button onclick="refreshDesktopWindows()" style="padding: 6px 12px; font-size: 11px;">🔍 Scan Windows</button>
    </div>
    <div class="deck-row">
      <select id="desktop-window-select" class="deck-select" style="flex: 1; min-width: 320px;">
        <option value="">-- Click "Scan Windows" to enumerate desktop HWNDs --</option>
      </select>
      <button onclick="captureSelectedWindow()">📸 Capture Window</button>
    </div>
    <div id="desktop-preview-container" style="display: none; margin-top: 14px; background: #030407; border: 1px solid var(--panel-border); border-radius: 6px; padding: 12px;">
      ...
      <img id="desktop-preview-img" style="max-width: 100%; max-height: 480px; border-radius: 4px; border: 1px solid #222; display: block; margin: 0 auto;" src="" alt="Desktop Snapshot" />
    </div>
  </div>
