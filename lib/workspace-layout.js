...
class WorkspaceLayoutManager {
  ...
  async applyLayout(layoutType, params = {}) {
    ...
    let { workX, workY, workW, workH } = work;

    if (params.monitorIndex !== undefined && Array.isArray(work.monitors) && work.monitors[params.monitorIndex]) {
      const mon = work.monitors[params.monitorIndex];
      workX = mon.workArea.x;
      workY = mon.workArea.y;
      workW = mon.workArea.width;
      workH = mon.workArea.height;
    } else if (params.monitorDevice && Array.isArray(work.monitors)) {
      const mon = work.monitors.find(m => m.device.toLowerCase().includes(params.monitorDevice.toLowerCase()));
      if (mon) {
        workX = mon.workArea.x;
        workY = mon.workArea.y;
        workW = mon.workArea.width;
