...
class GeminiDesktopBridge {
  ...
  async readDiscordMessages(targetChannel) {
    if (targetChannel) {
      await this.navigateDiscord(targetChannel);
      await new Promise(r => setTimeout(r, 400));
    }
    const snap = await this.deltaCapture("Discord", null, 0, 0.0);
    const snapPath = snap.path || snap.outputPath;
    if (!snapPath || !fs.existsSync(snapPath)) {
      return { success: false, error: "Failed to capture Discord snapshot", snap };
    }
    const ocrLines = await new Promise((resolve) => {
      execFile(this.ocrBinPath, ["json", snapPath], { timeout: 15000, maxBuffer: 10 * 1024 * 1024 }, (err, stdout, stderr) => {
        if (stdout && stdout.trim()) {
          try {
            const data = JSON.parse(stdout.trim());
