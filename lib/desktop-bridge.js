...
class GeminiDesktopBridge {
  ...
  async getSystemVitals() {
    return new Promise((resolve) => {
      execFile(this.binPath, ["vitals"], { timeout: 5000, encoding: "utf8" }, (err, stdout, stderr) => {
        if (stdout && stdout.trim()) {
          try {
            const parsed = JSON.parse(stdout.trim());
            if (parsed && parsed.success) return resolve(parsed);
  ...
  async getCursorInfo() {
    return new Promise((resolve) => {
      execFile(this.binPath, ["cursor"], { timeout: 5000, encoding: "utf8" }, (err, stdout, stderr) => {
        if (stdout && stdout.trim()) {
          try {
            const parsed = JSON.parse(stdout.trim());
            if (parsed && parsed.success) return resolve(parsed);
  ...
  async getAudioVolume() {
    return new Promise((resolve) => {
      execFile(this.binPath, ["volume"], { timeout: 5000, encoding: "utf8" }, (err, stdout, stderr) => {
        if (stdout && stdout.trim()) {
          try {
            const parsed = JSON.parse(stdout.trim());
            if (parsed && parsed.success) return resolve(parsed);
  ...
  async setAudioVolume(percent) {
    ...
    return new Promise((resolve) => {
      execFile(this.binPath, ["set_volume", String(p)], { timeout: 5000, encoding: "utf8" }, (err, stdout, stderr) => {
        if (stdout && stdout.trim()) {
          try {
            const parsed = JSON.parse(stdout.trim());
            if (parsed && parsed.success) return resolve(parsed);
  ...
  async setAudioMute(mute = true) {
    ...
    return new Promise((resolve) => {
      execFile(this.binPath, [cmd], { timeout: 5000, encoding: "utf8" }, (err, stdout, stderr) => {
        if (stdout && stdout.trim()) {
          try {
            const parsed = JSON.parse(stdout.trim());
            if (parsed && parsed.success) return resolve(parsed);
  ...
  async toggleAudioMute() {
    return new Promise((resolve) => {
      execFile(this.binPath, ["toggle_mute"], { timeout: 5000, encoding: "utf8" }, (err, stdout, stderr) => {
        if (stdout && stdout.trim()) {
          try {
            const parsed = JSON.parse(stdout.trim());
            if (parsed && parsed.success) return resolve(parsed);
  ...
  async getProcessVitals(pidOrName) {
    ...
    return new Promise((resolve) => {
      execFile(this.binPath, ["process_vitals", target], { timeout: 5000, encoding: "utf8" }, (err, stdout, stderr) => {
        if (stdout && stdout.trim()) {
          try {
            const parsed = JSON.parse(stdout.trim());
            if (parsed && parsed.success) return resolve(parsed);
