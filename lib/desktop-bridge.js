...
class GeminiDesktopBridge {
  ...
  async captureWindow(titleFilter, outputPath = null) {
    ...
    return new Promise((resolve, reject) => {
      execFile(this.binPath, ["capture", titleFilter, outputPath], { timeout: 15000 }, (err, stdout, stderr) => {
  ...
  async clickTarget(titleFilter, query, button = "left") {
    // Tier 1: Try semantic UIAutomation first (sub-15ms cached)
    const elRes = await this.clickElement(titleFilter, query, button);
    ...
    const ocrRes = await this.clickText(titleFilter, query, button);
  ...
  async navigateDiscord(target) {
    await this.focusWindow("Discord");
