...
// Vector projection / dimensional resampling helper
function projectVector(rawVector, targetDim) {
  if (!rawVector || rawVector.length === 0) return new Float32Array(targetDim);
  if (rawVector.length === targetDim) return new Float32Array(rawVector);
  const out = new Float32Array(targetDim);
  const step = rawVector.length / targetDim;
  for (let i = 0; i < targetDim; i++) {
    const start = Math.floor(i * step);
    const end = Math.min(rawVector.length, Math.floor((i + 1) * step));
    let sum = 0;
    let count = 0;
    ...
    out[i] = count > 0 ? sum / count : (rawVector[start] || 0);
  }
  return out;
}

// Normalizes vector to unit L2 sphere
...
/**
 * Generates true dense neural transformer embeddings with automatic multi-tier fallback:
 * 1. Local llama-server (OpenAI standard /v1/embeddings on port 11436)
 * 2. Ollama (/api/embeddings on port 11434)
 * 3. Gemini API (text-embedding-004 if GEMINI_API_KEY set)
 * 4. Deterministic harmonic unit-vector projection (zero-latency offline fallback)
 */
async function generateDenseEmbedding(text, dim = DEFAULT_EMBEDDING_DIM, options = {}) {
  ...
  try {
    ...
    const resp = await fetch("http://127.0.0.1:11436/v1/embeddings", {
      method: "POST",
    ...
    if (resp.ok) {
      ...
      if (Array.isArray(raw) && raw.length > 0) {
        const projected = projectVector(raw, dim);
  ...
  try {
    ...
    if (resp.ok) {
      ...
      if (Array.isArray(data.embedding) && data.embedding.length > 0) {
        const projected = projectVector(data.embedding, dim);
        normalizeL2(projected);
        return {
          vector: projected,
          source: "ollama",
          rawDim: data.embedding.length
        };
      }
    }
  } catch {}
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (Array.isArray(values) && values.length > 0) {
          const projected = projectVector(values, dim);
          normalizeL2(projected);
          return {
            vector: projected,
            source: "gemini-api",
            rawDim: values.length
          };
        }
      }
    } catch {}
  ...
  // 4. Deterministic Offline Harmonic Projection Fallback
  const fallback = generateSemanticEmbedding(clean, dim);
  return {
    vector: fallback,
    source: "offline-harmonic",
    rawDim: dim
  };
}
...
class HmbEngine {
  ...
  async remember({
    ...
    let emb = vector;
    let embSource = "custom";
    if (!emb || emb.length !== this.dim) {
      if (useDense) {
        const denseRes = await generateDenseEmbedding(`${concept} ${content} ${cleanCat}`, this.dim);
        emb = denseRes.vector;
        embSource = denseRes.source;
      } else {
        emb = generateSemanticEmbedding(`${concept} ${content} ${cleanCat}`, this.dim);
        embSource = "offline-harmonic";
      }
  ...
  async rememberDense(record) {
    return await this.remember({ ...record, useDense: true });
  }
  ...
  async recall({
    ...
    let queryVec = null;
    let querySource = "offline-harmonic";
    if (useDense) {
      const denseRes = await generateDenseEmbedding(cleanQuery, this.dim);
      queryVec = denseRes.vector;
      querySource = denseRes.source;
    } else {
      queryVec = generateSemanticEmbedding(cleanQuery, this.dim);
    }
    ...
    return {
      ...
      embeddingSource: querySource,
      memories: results
    };
  }

  async recallDense(params) {
    return await this.recall({ ...params, useDense: true });
  }

  getEmbeddingStatus() {
    return {
      dim: this.dim,
      transformerSupported: true,
      supportedSources: ["llama-server", "haven", "ollama", "gemini-api", "offline-harmonic"],
      defaultSource: "offline-harmonic"
    };
  }
  ...
  /**
   * Generates a 2D dimensional projection & synaptic graph of memory anchors
   * for the real-time Memory Galaxy visualization.
   */
  async getGalaxyMap({ category = null, limit = 100, vaultPath = null } = {}) {
    await this.initialize();
    if (vaultPath && vaultPath !== this.vaultPath) {
      this.loadFromHmb(vaultPath);
    }
    ...
    if (n === 0) return { nodes: [], links: [], totalNodes: 0, totalLinks: 0 };

    // Initial radial placement
    const nodes = sample.map((m, idx) => {
      const angle = (idx / (n || 1)) * 2 * Math.PI;
      const radius = 0.35 + (idx % 3) * 0.18;
      return {
        id: m.id.toString(),
        concept: m.concept,
        content: m.content.length > 120 ? m.content.substring(0, 117) + "..." : m.content,
        category: m.category,
        weight: m.weight,
        emotional_salience: m.emotional_salience,
        access_count: Number(m.access_count),
    ...
    // Compute pairwise cosine similarities and links
    const links = [];
    for (let i = 0; i < n; i++) {
      for (let j = i + 1; j < n; j++) {
        const sim = cosineSimilarity(nodes[i].vector, nodes[j].vector, this.dim);
        if (sim >= 0.28) {
          links.push({
            source: nodes[i].id,
            target: nodes[j].id,
    ...
    // Run 35 iterations of spring-force layout simulation for semantic clustering
    const kRepel = 0.008;
    const kAttract = 0.04;
    ...
    for (let step = 0; step < 35; step++) {
      ...
      for (const link of links) {
        ...
        if (i !== -1 && j !== -1) {
          ...
          const force = (dist - (1.0 - link.similarity) * 0.5) * kAttract * link.similarity;
          const fx = (dx / dist) * force;
          const fy = (dy / dist) * force;
    ...
    const scale = 0.85 / maxDist;
    for (const node of nodes) {
      node.x = Number((node.x * scale).toFixed(4));
      node.y = Number((node.y * scale).toFixed(4));
      delete node.vx;
      delete node.vy;
      delete node.vector;
    }
    ...
    const clusterMap = {};
    for (const node of nodes) {
      clusterMap[node.category] = (clusterMap[node.category] || 0) + 1;
    }
    const clusters = Object.entries(clusterMap).map(([category, count]) => ({
      category,
      count
    }));
