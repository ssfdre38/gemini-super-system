...
class GeminiAndroidGateway extends EventEmitter {
  ...
  _handleHttp(req, res) {
    ...
    // 5. Inbound Mobile Device Vitals (Battery, Charging, Screen)
    if (url.pathname === "/api/android/vitals" && req.method === "POST") {
      let body = "";
      req.on("data", (chunk) => (body += chunk));
      req.on("end", () => {
        try {
          const parsed = JSON.parse(body || "{}");
          const devId = parsed.deviceId || "android-client";
          
          let dev = null;
          for (const d of this.devices.values()) {
            if (d.deviceId === devId) { dev = d; break; }
          }
          if (!dev) {
            dev = new AndroidDevice({ remoteAddress: req.socket.remoteAddress }, req);
            dev.deviceId = devId;
          }
          dev.updateVitals(parsed);

          // Reflex: Low Battery Alert (<15% & not charging)
          if (dev.battery && dev.battery.percent > 0 && dev.battery.percent <= 15 && !dev.battery.isCharging) {
            if (this.orchestrator && this.orchestrator.bus) {
              this.orchestrator.bus.emitNarration(
                `⚠️ Battery Alert: ${dev.deviceName || dev.model} battery is critical at ${dev.battery.percent}%.`,
                "alert",
                { deviceId: dev.deviceId, battery: dev.battery }
              );
            }
          }

          // Reflex: Docked & Charging -> Avatar Cozy
          if (dev.battery && dev.battery.isCharging) {
            if (this.orchestrator && this.orchestrator.gemmiBridge) {
              this.orchestrator.gemmiBridge.setThought(`${dev.deviceName || "Mobile tablet"} docked and charging.`);
            }
          }

          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ ok: true, success: true, vitals: dev.toJSON() }));
        } catch (err) {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ ok: false, error: err.message }));
        }
    ...
    // 6. Mobile Optical Intake (Camera snapshot upload)
    if (url.pathname === "/api/android/optical" && req.method === "POST") {
      const chunks = [];
      ...
      req.on("end", async () => {
        try {
          ...
          // Check if payload is JSON with base64 or raw image
          ...
          if (contentType.includes("application/json")) {
            ...
            imageBuffer = buffer;
            metadata = {
              landmark: req.headers["x-landmark"] || "Mobile Snapshot",
              latitude: parseFloat(req.headers["x-latitude"]) || null,
              longitude: parseFloat(req.headers["x-longitude"]) || null,
              deviceId: req.headers["x-device-id"] || "android-client"
            };
          }

          if (!imageBuffer || imageBuffer.length === 0) {
            res.writeHead(400, { "Content-Type": "application/json" });
            return res.end(JSON.stringify({ ok: false, error: "Empty or invalid image payload" }));
          }

          const filename = `capture_${Date.now()}_${crypto.randomBytes(3).toString("hex")}.jpg`;
          const filePath = path.join(CONFIG.paths.capturesDir, filename);

          fs.writeFileSync(filePath, imageBuffer);

          const landmarkText = metadata.landmark || "Field Station";
          if (this.orchestrator && this.orchestrator.bus) {
            this.orchestrator.bus.emitNarration(
              `📸 Mobile Optical Ingest: Received snapshot (${(imageBuffer.length / 1024).toFixed(1)} KB) near ${landmarkText}.`,
              "optical",
              { filename, size: imageBuffer.length, metadata }
            );
          }

          // Ingest into 64-bit Haven Memory Bank (.hmb)
          if (this.orchestrator && this.orchestrator.hmb) {
            await this.orchestrator.hmb.remember({
              concept: `Visual Capture: ${landmarkText}`,
              content: `Optical snapshot captured by mobile client near ${landmarkText}. File: ${filename}, size: ${imageBuffer.length} bytes.`,
              category: "OPTICAL",
              weight: 0.90,
          ...
          // Trigger Avatar thought & gesture
          if (this.orchestrator && this.orchestrator.gemmiBridge) {
            this.orchestrator.gemmiBridge.setThought(`Examining visual snapshot from ${landmarkText}.`);
            this.orchestrator.gemmiBridge.triggerAction("nod");
          }

          this.emit("optical_captured", { filename, filePath, size: imageBuffer.length, metadata });
