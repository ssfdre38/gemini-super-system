...
class GemmiBridge extends EventEmitter {
  constructor(orchestrator = null, options = {}) {
    ...
    this.avatarPort = parseInt(options.avatarPort || process.env.GEMMI_AVATAR_PORT || CONFIG.ports.avatar, 10);
    this.meshPort = parseInt(options.meshPort || process.env.GEMMI_MESH_PORT || CONFIG.ports.mesh, 10);
  ...
  _handleAvatarHttp(req, res) {
    ...
    // Direct REST Animate / Locomotion trigger on Port 8088
    if (url.pathname === "/api/animate" && req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          ...
          if (data.state) this.setLocomotion(data.state);
          if (data.action) this.triggerAction(data.action);
          if (data.thought) this.setThought(data.thought);
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(
            JSON.stringify({
              success: true,
    ...
    // Direct GPS Telemetry inspector on Port 8088
    if (url.pathname === "/api/gps" && req.method === "GET") {
      res.writeHead(200, { "Content-Type": "application/json" });
      return res.end(JSON.stringify({ ok: true, gps: this.latestGpsTelemetry }, null, 2));
    }
    ...
    if (url.pathname === "/api/mesh/state" && req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          ...
          // Cross-Device Autonomous Locomotion & Cognition Reflex
          const spd = this.latestGpsTelemetry.speed;
          if (spd > 1.2 && this.currentLocomotion !== "walk") {
            this.setLocomotion("walk");
          } else if (spd <= 0.2 && this.currentLocomotion === "walk") {
            this.setLocomotion("cozy");
          }

          // Contextual monologue thought reflex
          const lm = this.latestGpsTelemetry.landmark;
          if (lm && !lm.includes("Awaiting")) {
            const spdText = spd > 0.5 ? ` (moving at ${spd.toFixed(1)} m/s)` : "";
            this.setThought(`Active near ${lm}${spdText}.`);
          }

          // Ground into 64-bit Haven Memory Bank (.hmb)
          if (this.orchestrator && this.orchestrator.hmb && lm && !lm.includes("Awaiting")) {
            this.orchestrator.hmb.remember({
              concept: `Spatial Telemetry: ${lm}`,
              content: `Mobile GPS telemetry logged at ${lm} (lat: ${this.latestGpsTelemetry.latitude.toFixed(4)}, lng: ${this.latestGpsTelemetry.longitude.toFixed(4)}, speed: ${spd.toFixed(1)} m/s, bearing: ${Math.round(this.latestGpsTelemetry.bearing)}°).`,
              category: "LOCATION",
              weight: 0.85,
