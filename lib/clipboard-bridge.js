...
class ClipboardBridge {
  ...
  async getText() {
    return new Promise((resolve) => {
      execFile(this.binPath, ["clip_get"], { timeout: 4000, encoding: "utf8" }, (err, stdout) => {
  ...
  async setText(text) {
    ...
    return new Promise((resolve) => {
      execFile(this.binPath, ["clip_set", text], { timeout: 4000, encoding: "utf8" }, (err, stdout) => {
        if (stdout && stdout.trim()) {
          try {
            const parsed = JSON.parse(stdout.trim());
            return resolve(parsed);
  ...
  async saveImage(outputPath) {
    ...
    return new Promise((resolve) => {
      execFile(this.binPath, ["clip_save_image", resolvedPath], { timeout: 6000, encoding: "utf8" }, (err, stdout) => {
        if (stdout && stdout.trim()) {
          try {
            const parsed = JSON.parse(stdout.trim());
            return resolve(parsed);
  ...
  async loadImage(inputPath) {
    ...
    return new Promise((resolve) => {
      execFile(this.binPath, ["clip_load_image", resolvedPath], { timeout: 6000, encoding: "utf8" }, (err, stdout) => {
        if (stdout && stdout.trim()) {
          try {
            const parsed = JSON.parse(stdout.trim());
            return resolve(parsed);
  ...
  async clear() {
    return new Promise((resolve) => {
      execFile(this.binPath, ["clip_clear"], { timeout: 4000, encoding: "utf8" }, (err, stdout) => {
        if (stdout && stdout.trim()) {
          try {
            const parsed = JSON.parse(stdout.trim());
            return resolve(parsed);
