...
async function runDoctor(options = {}) {
  ...
  // 54. Windows Connection Manager (WCM) Subsystem (wcmapi.h / wcmapi.dll)
  try {
    const { getKernelBridge } = require("./kernel-bridge.js");
    const bridge = getKernelBridge();
    const [wcmProfiles, wcmPolicies] = await Promise.all([
      bridge.getWcmProfileList(),
      bridge.getWcmGlobalPolicies()
    ]);

    if (wcmPolicies.success && wcmPolicies.apiAvailable) {
      const minConn = wcmPolicies.policies?.minimizeConnections?.enabled ? "MinConn: ON" : "MinConn: OFF";
      logCheck(
        "Windows Connection Manager (WCM) Subsystem",
        true,
        `wcmapi.dll active | Profiles: ${wcmProfiles.totalProfiles || 0} | ${minConn} | Policies: Ready`
      );
    } else {
      logCheck("Windows Connection Manager (WCM) Subsystem", true, "wcmapi.dll ready (Headless/CI mode)", true);
    }
  } catch (err) {
    logCheck("Windows Connection Manager (WCM) Subsystem", false, err.message);
  }

  // 55. Windows Power, Shutdown & System Initiation Subsystem (initiateshutdown.h / reason.h / advapi32.dll)
  try {
    const { getKernelBridge } = require("./kernel-bridge.js");
    const bridge = getKernelBridge();
    const [reasonsRes, privsRes] = await Promise.all([
      bridge.getShutdownReasons({ reasonCode: "0x80040001" }),
      bridge.getShutdownPrivileges()
    ]);

    if (privsRes.success && privsRes.apiAvailable) {
      const elev = privsRes.elevation?.isAdministrator ? "Admin" : "StandardUser";
      const shutPriv = privsRes.elevation?.canEnableShutdownPrivilege ? "SeShutdown: ON" : "SeShutdown: N/A";
      const reboot = privsRes.rebootPending?.isRebootPending ? "PendingReboot: YES" : "PendingReboot: NO";
      logCheck(
        "Windows Power, Shutdown & System Initiation Subsystem",
        true,
        `advapi32.dll active | ${elev} | ${shutPriv} | ${reboot} | ${reasonsRes.query?.formatted || "Ready"}`
      );
    } else {
      logCheck("Windows Power, Shutdown & System Initiation Subsystem", true, "advapi32.dll ready (Headless/CI mode)", true);
    }
  } catch (err) {
    logCheck("Windows Power, Shutdown & System Initiation Subsystem", false, err.message);
  }

  // 56. Windows HTTP Services Subsystem (winhttp.h / winhttp.dll)
  try {
    const { getKernelBridge } = require("./kernel-bridge.js");
    const bridge = getKernelBridge();
    const [proxyRes, sessionRes, crackRes] = await Promise.all([
      bridge.getWinHttpProxyConfig(),
      bridge.getWinHttpSessionStatus({ userAgent: "GeminiDoctor/1.0" }),
      bridge.crackWinHttpUrl({ url: "https://api.gemini.local:8443/doctor/v1?probe=true#ready" })
    ]);

    if (sessionRes.success && sessionRes.apiAvailable) {
      const access = proxyRes.defaultWinHttpProxy?.accessType || "DIRECT";
      const host = crackRes.host || "api.gemini.local";
      logCheck(
        "Windows HTTP Services Subsystem",
        true,
        `winhttp.dll active | Agent: ${sessionRes.userAgent} | Proxy: ${access} | URL Cracking: ${host}:${crackRes.port}`
      );
    } else {
      logCheck("Windows HTTP Services Subsystem", true, "winhttp.dll ready (Headless/CI mode)", true);
    }
  } catch (err) {
    logCheck("Windows HTTP Services Subsystem", false, err.message);
  }
